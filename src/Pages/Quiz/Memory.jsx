...
const Memory = () => {
    ...
    const [moves, setMoves] = useState(0); // New state for tracking moves
    ...
    const handleCardClick = (index, name) => {
        if (flippedIndices.length < 2 && !flippedIndices.includes(index) && !matchedPairs.has(name)) {
            ...
            // Increment moves only after the second card is flipped
            if (flippedIndices.length === 1) {
                ...
                if (firstName === name) {
                    ...
                    setTimeout(() => {
                        setFlippedIndices([]);
                    }, 1000); // Reset flipped cards after 1 second
                }
            }
        }
    };
    ...
    return (
        <>
            <Box bg='teal' p='20'>
                ...
                <Text fontSize='xl' color='white' mb='4'>Moves: {moves}</Text> {/* Display moves */}
